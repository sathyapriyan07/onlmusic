...
import { Search, ArrowLeft, Check, Music2 } from "lucide-react";
...
export default function ImportArtistsPage() {
  ...
  const [artists] = useState<{ name: string }[]>([]);
  ...
  async function search() {
    ...
    try {
      ...
      if (searchSource === "itunes") {
        ...
        res = await fetch(`https://api.deezer.com/search/artist?q=${term}&limit=25`);
        const data = await res.json();
        const mapped: ItunesArtist[] = (data.data ?? []).map((r: Record<string, unknown>) => ({
  ...
  async function importSelected() {
    ...
    try {
      const toImport = results.filter(r => selectedIds.has(r.artistId));
  ...
  return (
    <div>
      <Helmet>
        <title>Import Artists · Admin</title>
      </Helmet>

      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate("/admin/artists")} className="p-2 rounded-lg hover:bg-white/10">
          <ArrowLeft className="w-5 h-5 text-[var(--muted)]" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-[var(--text)]">Import Artists</h1>
      ...
      {/* Search Section */}
      <AdminCard title="Search Artists">
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <AdminButton onClick={importSelected} disabled={selectedIds.size === 0 || importing}>
              {importing ? "Importing..." : `Import ${selectedIds.size} Artists`}
            </AdminButton>
          </div>
          ...
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {results.map((r) => (
              <div
                key={r.artistId}
                onClick={() => toggleSelect(r.artistId)}
                className={`flex items-center gap-4 p-4 rounded-xl cursor-pointer transition-colors ${
                  selectedIds.has(r.artistId) ? "bg-[var(--accent)]/20" : "bg-panel hover:bg-white/5"
                }`}
              >
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                  selectedIds.has(r.artistId) ? "border-[var(--accent)] bg-[var(--accent)]" : "border-[var(--muted)]"
                }`}>
                  {selectedIds.has(r.artistId) && <Check className="w-4 h-4 text-black" />}
                </div>
                <div className="flex-1">
                  <p className="font-medium text-[var(--text)]">{r.artistName}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {results.length === 0 && !searching && !query && (
        <div className="mt-6">
          <div className="text-center py-12">
            <Music2 className="w-12 h-12 text-[var(--muted)] mx-auto mb-4" />
            <p className="text-[var(--muted)]">Search for artists to import</p>
          </div>
        </div>
      )}
    </div>
