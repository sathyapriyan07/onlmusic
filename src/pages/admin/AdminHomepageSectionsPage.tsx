        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          <div className="rounded-3xl border border-app bg-white/5 p-4">
            ...
            <div className="mt-3 space-y-3">
              <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title (Trending Songs, Popular Albums…)" className="w-full rounded-2xl border border-app bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-zinc-500 backdrop-blur" />

              <div className="grid grid-cols-2 gap-2">
                <select value={type} onChange={(e) => { setType(e.target.value as HomepageSectionType); setItems([]); }} className="w-full rounded-2xl border border-app bg-white/5 px-4 py-3 text-sm text-white outline-none backdrop-blur">
                  <option value="songs">Songs</option>
                  <option value="albums">Albums</option>
                  <option value="artists">Artists</option>
                </select>
                <input value={order} onChange={(e) => setOrder(e.target.value)} placeholder="Order (0..)" className="w-full rounded-2xl border border-app bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-zinc-500 backdrop-blur" />
              </div>
              ...
              <div className="rounded-2xl border border-app bg-white/5 p-3">
                ...
                <select
                  ...
                  className="w-full rounded-2xl border border-app bg-white/5 px-3 py-2 text-sm text-white outline-none backdrop-blur"
                >
                ...
                <div className="mt-3 space-y-2">
                  {items.map((id) => {
                    ...
                    return (
                      <div key={id} className="flex items-center justify-between gap-3 rounded-2xl border border-app bg-white/5 px-3 py-2 text-sm">
          ...
          <div className="rounded-3xl border border-app bg-white/5 p-4">
            ...
            {loading ? (
              ...
              <div className="mt-3 max-h-[520px] space-y-2 overflow-auto pr-2">
                {sections.map((s) => (
                  <div key={s.id} className="rounded-2xl border border-app bg-white/5 p-3">
