        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          <div className="rounded-3xl border border-app bg-white/5 p-4">
            ...
            <div className="mt-3 space-y-3">
              <select value={entityType} onChange={(e) => { setEntityType(e.target.value as LinkEntityType); setEntityId(""); }} className="w-full rounded-2xl border border-app bg-white/5 px-4 py-3 text-sm text-white outline-none backdrop-blur">
                <option value="song">Song</option>
                <option value="album">Album</option>
                <option value="artist">Artist</option>
              </select>

              <select value={entityId} onChange={(e) => setEntityId(e.target.value)} className="w-full rounded-2xl border border-app bg-white/5 px-4 py-3 text-sm text-white outline-none backdrop-blur">
              ...
              <input value={platform} onChange={(e) => setPlatform(e.target.value)} placeholder="Platform (YouTube, Spotify…)" className="w-full rounded-2xl border border-app bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-zinc-500 backdrop-blur" />
              <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="URL" className="w-full rounded-2xl border border-app bg-white/5 px-4 py-3 text-sm text-white outline-none placeholder:text-zinc-500 backdrop-blur" />
          ...
          <div className="rounded-3xl border border-app bg-white/5 p-4">
            ...
            {loading ? (
              ...
              <div className="mt-3 max-h-[520px] space-y-2 overflow-auto pr-2">
                {links.map((l) => (
                  <div key={l.id} className="rounded-2xl border border-app bg-white/5 p-3">
