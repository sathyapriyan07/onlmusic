        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-2 sm:p-4 overflow-auto">
          <div className="w-full max-w-2xl rounded-2xl border border-app bg-panel p-4 sm:p-6 my-4">
            ...
            <div className="mt-4 space-y-3">
              <select value={bulkEntityType} onChange={(e) => { setBulkEntityType(e.target.value as LinkEntityType); setBulkEntityId(""); }} className="w-full rounded-lg border border-app bg-input px-4 py-3 text-sm text-[var(--text)] outline-none">
                <option value="song">Song</option>
                <option value="album">Album</option>
                <option value="artist">Artist</option>
              </select>
              <select value={bulkEntityId} onChange={(e) => setBulkEntityId(e.target.value)} className="w-full rounded-lg border border-app bg-input px-4 py-3 text-sm text-[var(--text)] outline-none">
              ...
              <div className="space-y-2 max-h-60 overflow-auto">
                {bulkLinks.map((link, i) => (
                  <div key={i} className="flex gap-2 items-start">
                    <select value={link.category} onChange={(e) => updateBulkLinkRow(i, "category", e.target.value)} className="w-24 rounded-lg border border-app bg-input px-2 py-2 text-sm text-[var(--text)] outline-none">
                      <option value="official">Official</option>
                    ...
                    <input value={link.platform} onChange={(e) => updateBulkLinkRow(i, "platform", e.target.value)} placeholder="Platform" className="flex-1 rounded-lg border border-app bg-input px-3 py-2 text-sm text-[var(--text)] outline-none" />
                    <input value={link.url} onChange={(e) => updateBulkLinkRow(i, "url", e.target.value)} placeholder="URL" className="flex-1 rounded-lg border border-app bg-input px-3 py-2 text-sm text-[var(--text)] outline-none" />
                    <button type="button" onClick={() => removeBulkLinkRow(i)} className="shrink-0 text-red-400 hover:text-red-300 p-2">✕</button>
                  </div>
                ))}
              </div>
              <button type="button" onClick={addBulkLinkRow} className="text-sm text-[var(--accent)] hover:underline">+ Add Link</button>
              <div className="flex gap-2 pt-2">
                <button type="button" disabled={!bulkEntityId || bulkLinks.length === 0 || bulkSaving} onClick={saveBulkLinks} className="btn-primary rounded-lg px-4 py-3 text-sm font-semibold disabled:opacity-50">
                  {bulkSaving ? "Saving..." : "Save All Links"}
                </button>
                <button type="button" onClick={() => setBulkModalOpen(false)} className="btn-secondary rounded-lg px-4 py-3 text-sm">Cancel</button>
              </div>
            </div>
          </div>
        </div>
